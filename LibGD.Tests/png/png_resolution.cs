public static class GlobalMembersPng_resolution
{
	#if __cplusplus
	#endif
	#define GD_H
	...
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
		if (GlobalMembersGdtest._gd.gdTestAssert(__FILE__, __LINE__, "assert failed in <%s:%i>\n", ((im).res_x == 72)) == 0 || GlobalMembersGdtest._gd.gdTestAssert(__FILE__, __LINE__, "assert failed in <%s:%i>\n", ((im).res_y == 300)) == 0)
		{
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			(...) GlobalMembersGdtest._gd.gdTestErrorMsg(__FILE__, __LINE__, __VA_ARGS__)("failed image resolution X (%d != 72) or Y (%d != 300)\n", (im).res_x, (im).res_y);
			gd.gdImageDestroy(im);
			return 1;
		}
