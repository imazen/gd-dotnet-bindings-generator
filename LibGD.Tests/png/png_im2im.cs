...
public static class GlobalMembersPng_im2im
{
	#if __cplusplus
	#endif
	#define GD_H
	...
	//C++ TO C# CONVERTER NOTE: The following #define macro was replaced in-line:
	//ORIGINAL LINE: #define OUTPUT_PNG(name) do { FILE *fp; fp = fopen("png_im2im_" #name ".png", "wb"); if (fp) { gd.gdImagePng(name, fp); fclose(fp); } } while (0)
	#define OUTPUT_PNG

		do
		{
			FILE fp;
			fp = fopen("png_im2im_" "src" ".png", "wb");
			if (fp != null)
			{
				gd.gdImagePng(src, fp);
				fclose(fp);
			}
		} while (0);
		p = gd.gdImagePngPtr(src, size);
		if (p == null)
		{
			status = 1;
		...
		dst = gd.gdImageCreateFromPngPtr(size, p);
		if (dst == null)
		{
			status = 1;
		...
		{
			FILE fp;
			fp = fopen("png_im2im_" "dst" ".png", "wb");
			if (fp != null)
			{
				gd.gdImagePng(dst, fp);
				fclose(fp);
			}
		} while (0);
