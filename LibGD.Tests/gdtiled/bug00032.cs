...
public static class GlobalMembersBug00032
{
	#if __cplusplus
	#endif
	#define GD_H
	...
	static int Main()
	{
		gdImageStruct im;
		gdImageStruct tile;
		string path = new string(new char[GlobalMembersGdtest.DefineConstants.GDTEST_STRING_MAX]);

		gd.gdSetErrorMethod(GlobalMembersGdtest.gdSilence);

	#if snprintf_ConditionalDefinition1
		_snprintf(path, DefineConstants.GDTEST_STRING_MAX - 1, "%s/gdtiled/%s", DefineConstants.GDTEST_TOP_DIR, DefineConstants.exp_img);
	#else
		snprintf(path, GlobalMembersGdtest.DefineConstants.GDTEST_STRING_MAX - 1, "%s/gdtiled/%s", GlobalMembersGdtest.DefineConstants.GDTEST_TOP_DIR, GlobalMembersGdtest.DefineConstants.exp_img);
	#endif
	...
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
		if (GlobalMembersGdtest.gd.gdTestImageCompareToFile(__FILE__, __LINE__, null, (path), (im)) == 0)
		{
