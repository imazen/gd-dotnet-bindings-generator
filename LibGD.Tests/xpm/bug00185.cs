...
public static class GlobalMembersBug00185
{
	/* $Id$ */
	#if __cplusplus
	#endif
	...
	static int Main()
	{
		...
		path = string.Format("{0}/xpm/bug00185.xpm", DefineConstants.GDTEST_TOP_DIR);
		im = gd.gdImageCreateFromXpm(path);
		if (im == null)
			return 1;
		gd.gdImageDestroy(im);

		path = string.Format("{0}/xpm/bug00185_damaged.xpm", DefineConstants.GDTEST_TOP_DIR);
		im = gd.gdImageCreateFromXpm(path);
		if (im == null)
			return 0;
		gd.gdImageDestroy(im);
