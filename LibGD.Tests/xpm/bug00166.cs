public static class GlobalMembersBug00166
{
	/* $Id$ */
	#if __cplusplus
	#endif
	...
	static int Main()
	{
		...
		path = string.Format("{0}/xpm/bug00166.xpm", DefineConstants.GDTEST_TOP_DIR);
		im = gd.gdImageCreateFromXpm(path);
		if (im == null)
		{
			return 2;
		}
		c = gd.gdImageGetPixel(im, 1, 1);
		if (((im).trueColor != 0 ? (((c) & 0xFF0000) >> 16) : (im).red[(c)]) == 0xAA && ((im).trueColor != 0 ? (((c) & 0x00FF00) >> 8) : (im).green[(c)]) == 0xBB && ((im).trueColor != 0 ? ((c) & 0x0000FF) : (im).blue[(c)]) == 0xCC)
		{
			result = 0;
		}
		else
