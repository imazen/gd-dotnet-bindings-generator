public static class GlobalMembersBug00006
{
	#if __cplusplus
	#endif
	#define GD_H
	...
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			(...) GlobalMembersGdtest._gd.gdTestErrorMsg(__FILE__, __LINE__, __VA_ARGS__)("Cannot create image\n");
			return 1;
		}
		...
		fp = fopen(DefineConstants.TMP_FN, "wb");
		if (fp == null)
		{
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			(...) GlobalMembersGdtest._gd.gdTestErrorMsg(__FILE__, __LINE__, __VA_ARGS__)("Cannot open <%s> for writing\n", DefineConstants.TMP_FN);
			return 1;
		}
		gd.gdImageGif(im,fp);
		fclose(fp);
		...
		fp = fopen(DefineConstants.TMP_FN, "rb");
		if (fp == null)
		{
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			(...) GlobalMembersGdtest._gd.gdTestErrorMsg(__FILE__, __LINE__, __VA_ARGS__)("Cannot open <%s> for reading\n", DefineConstants.TMP_FN);
			return 1;
		}
		...
		{
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			(...) GlobalMembersGdtest._gd.gdTestErrorMsg(__FILE__, __LINE__, __VA_ARGS__)("Cannot create image from <%s>\n", DefineConstants.TMP_FN);
			return 1;
		}

		trans_c_f = ((im).transparent);
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
		if (GlobalMembersGdtest._gd.gdTestAssert(__FILE__, __LINE__, "assert failed in <%s:%i>\n", (trans_c_f == 1)) != 0)
		{
	...
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			if (GlobalMembersGdtest._gd.gdTestAssert(__FILE__, __LINE__, "assert failed in <%s:%i>\n", (r_f == r)) == 0 || GlobalMembersGdtest._gd.gdTestAssert(__FILE__, __LINE__, "assert failed in <%s:%i>\n", (g_f == g)) == 0 || GlobalMembersGdtest._gd.gdTestAssert(__FILE__, __LINE__, "assert failed in <%s:%i>\n", (b_f == b)) == 0)
			{
				error = 1;
			}
		}
		...
		/* Destroy it */
		gd.gdImageDestroy(im);
		if (remove(DefineConstants.TMP_FN) == -1)
		{
	//C++ TO C# CONVERTER TODO TASK: There is no direct equivalent in C# to the following C++ macro:
			(...) GlobalMembersGdtest._gd.gdTestErrorMsg(__FILE__, __LINE__, __VA_ARGS__)("Cannot remove file: <%s>\n", DefineConstants.TMP_FN);
		}
		return error;
	}
}
