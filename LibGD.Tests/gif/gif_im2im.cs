...
public static class GlobalMembersGif_im2im
{
	#if __cplusplus
	#endif
	#define GD_H
	...
	//C++ TO C# CONVERTER NOTE: The following #define macro was replaced in-line:
	//ORIGINAL LINE: #define OUTPUT_GIF(name) do { FILE *fp; fp = fopen("gif_im2im_" #name ".gif", "wb"); if (fp) { gd.gdImageGif(name, fp); fclose(fp); } } while (0)
	#define OUTPUT_GIF

		do
		{
			FILE fp;
			fp = fopen("gif_im2im_" "src" ".gif", "wb");
			if (fp != null)
			{
				gd.gdImageGif(src, fp);
				fclose(fp);
			}
		} while (0);
		...
		{
			FILE fp;
			fp = fopen("gif_im2im_" "dst" ".gif", "wb");
			if (fp != null)
			{
				gd.gdImageGif(dst, fp);
				fclose(fp);
			}
		} while (0);
